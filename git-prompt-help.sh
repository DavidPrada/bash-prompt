...
These are examples of the git prompt:
  ...
  ${GIT_PROMPT_PREFIX}${GIT_PROMPT_BRANCH}master${ResetColor}${GIT_PROMPT_REMOTE}↑·3${ResetColor}${GIT_PROMPT_SEPARATOR}${GIT_PROMPT_CHANGED}1${ResetColor}${GIT_PROMPT_SUFFIX}${ResetColor}  - on branch "master", ahead of remote by 3 commits, 1
                    file changed but not staged

  ${GIT_PROMPT_PREFIX}${GIT_PROMPT_BRANCH}status${ResetColor}${GIT_PROMPT_SEPARATOR}${GIT_PROMPT_STAGED}2${ResetColor}${GIT_PROMPT_SUFFIX}${ResetColor}     - on branch "status", 2 files staged

  ${GIT_PROMPT_PREFIX}${GIT_PROMPT_BRANCH}master${ResetColor}${GIT_PROMPT_SEPARATOR}${GIT_PROMPT_CHANGED}7${GIT_PROMPT_UNTRACKED}${ResetColor}${GIT_PROMPT_SUFFIX}${ResetColor}    - on branch "master", 7 files changed, some files untracked

  ${GIT_PROMPT_PREFIX}${GIT_PROMPT_BRANCH}master${ResetColor}${GIT_PROMPT_SEPARATOR}${GIT_PROMPT_CONFLICTS}2${GIT_PROMPT_CHANGED}3${ResetColor}${GIT_PROMPT_SUFFIX}${ResetColor}   - on branch "master", 2 conflicts, 3 files changed

  ${GIT_PROMPT_PREFIX}${GIT_PROMPT_BRANCH}master${ResetColor}${GIT_PROMPT_SEPARATOR}${GIT_PROMPT_STASHED}2${ResetColor}${GIT_PROMPT_SUFFIX}${ResetColor}     - on branch "master", 2 stash entries

  ${GIT_PROMPT_PREFIX}${GIT_PROMPT_BRANCH}experimental${ResetColor}${GIT_PROMPT_REMOTE}↓·2↑·3${ResetColor}${GIT_PROMPT_SEPARATOR}${GIT_PROMPT_CLEAN}${ResetColor}${GIT_PROMPT_SUFFIX}${ResetColor}
                  -  on branch "experimental"; your branch has diverged
                     by 3 commits, remote by 2 commits; the repository is
                     otherwise clean

  ${GIT_PROMPT_PREFIX}${GIT_PROMPT_BRANCH}:70c2952${ResetColor}${GIT_PROMPT_SEPARATOR}${GIT_PROMPT_CLEAN}${ResetColor}${GIT_PROMPT_SUFFIX}${ResetColor}    - not on any branch; parent commit has hash "70c2952"; the
